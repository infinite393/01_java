...
public class Application {
    public static void main(String[] args) {
        ...
        * 1. 새로운 클래스르를 작성할 시 기존에 작성한 클래스를 재사용할 수 있다
        ...
        * 1. 부모 클래스의 기능을 추가/변경할 시 자식크래스가 정상적으로 동작하는지에 대한 예측이 힘들다
        * 상속구조가 복잡해질수록 그 영향에 대한 예측이 힘들어 이런 단점이 유지보수성이 증가된다는 장점과는 반대로 유지보수에 악영향을 미친다
        * 2. 또한 부모 클래스의 변경 또한 쉽비 않다.
        * 자식 클래스에서 중요하게 사용하는 기능인 경우 부모클래스를 변경할 시 자식 클래스 모두 영향을 줄 수 있다
        * 역시 유지보수에 악영향을 미친다
        * 3. 부모 클래스에서는 의미 있었던 기능이 자식 클래스에서는 무의미할 수 있다(불필요한 기능)
        *
        * 장점과 단점을 고려했을 때
        * 상속은 재사용이라는 장점만 바라보게 되면 오용 가능성이 있기 때문에 유지보수에 좋지 않은 코드를 작성할 수 있따
        * 상속은 IS-A 관계로 구분이 되는 경우에만 사용해야 한다
        *   ex) 사람은동물이다. 개는 동물이다 -> 반대는?
        */
        ...
        */
        ...
        //FireCar fireCar1 = reacingCar(); 자식끼리는 상속안됨
