...
public class Applicatino {
    public static void main(String[] args) {
        ...
        * 추상클래스는ㄴ 선언부에  abstract 키워드를 명시해야 한다
        * 추상클래스로는 인스턴스르르 생성할 수 없다
        * 추상클래스를 ㄹ사용하려면 추상 클래스를 상속받은 하위 클래스르를 이용해서 인스턴스를 생성해야 한다
        * 이 때 추상클랠스는 상위 타입으로 사용될 수 있으면, 다형성을 이용할 수 있다
        *
        ...
        * 추상클래스느를 상속받아 구현할 때는 extends 키워드를 사용하며
        * 자바에서는  extends 로 클래스를 상속받을 시 하나의 부모 클래스만 가질 수 있다(단일 상속)
        *
        * 추상 메소드란?
        * 메소드의 선언부만 있고 구현부가 없는 메소드르르 추상 메소드라고 한다
